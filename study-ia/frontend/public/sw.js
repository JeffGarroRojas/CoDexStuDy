...
const CACHE_NAME = `study-ia-${CACHE_VERSION}`;
...
const API_CACHE_NAME = `study-ia-api-${CACHE_VERSION}`;
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames
          .filter((name) => name.startsWith('study-ia-') && name !== CACHE_NAME && name !== API_CACHE_NAME)
...
self.addEventListener('push', (event) => {
  ...
  const title = data.title || 'Study-IA';
  const options = {
